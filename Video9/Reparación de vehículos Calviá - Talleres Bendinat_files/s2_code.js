...
/* TimeParting Config */
//s.dstStart="3/27/2011"; // update to the correct Daylight Savings Time start
//s.dstEnd="10/30/2011"; // update to the correct Daylight Savings Time end date
//s.currentYear="2011"; // update to the current year (can be done programmaticaly) NO OLVIDAR
...
//	if(!s.campaign)
//		s.campaign=s.getQueryParam('ext');
//	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
//	
//	if(!s.eVar1)
//		s.eVar1=s.getQueryParam('int');
...
//	
//	s.prop5=s.getTimeParting('h','+1'); // Set hour 
//	s.prop6=s.getTimeParting('d','+1'); // Set day 
//	s.prop7=s.getTimeParting('w','+1'); // Set weekday
//	
...
//s.visitorNamespace="yellspain";
//s.trackingServer="yellspain.112.2o7.net";

/** Global analytics framework integration START **/

var gConf = typeof HAF !== "undefined" ? HAF.config.global : yellAnalyticsConfig.global;
var oConf = typeof HAF !== "undefined" ? HAF.config.omniture : yellAnalyticsConfig.omniture;
var s=s_gi(oConf.account);

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.charSet=gConf.charset;
/* Conversion Config */
s.currencyCode=gConf.currency;
/* Link Tracking Config */
s.trackDownloadLinks=gConf.trackDownloadLinks;
s.trackExternalLinks=gConf.trackExternalLinks;
s.trackInlineStats=true;
s.linkDownloadFileTypes=gConf.downloadFileTypes;
/*
(function(){		return;
	var flag = 'noTrackFlag';
	gConf.noTrackFlag = flag;
	var internalDomains = gConf.internalDomains;
	if(null != internalDomains && '' !== internalDomains) {
		internalDomains += ',' + flag;
	}
	else internalDomains = flag;
	s.linkInternalFilters = internalDomains;
})();*/
s.linkInternalFilters = gConf.internalDomains;
...
/* TimeParting Config */
s.dstStart=oConf.dstStart;
s.dstEnd=oConf.dstEnd; 
s.currentYear=oConf.currentYear;

s.usePlugins=false;  // Set to 'true' if s_doPlugins(s) needs to be called
...
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace=oConf.namespace;
s.trackingServer=oConf.server;
if ("serverSecure" in oConf) {
	s.trackingServerSecure = oConf.serverSecure;
}

/** Global analytics framework integration END **/
